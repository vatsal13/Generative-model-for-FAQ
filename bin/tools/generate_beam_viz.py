...
def main():
  ...
  for idx in range(len(beam_data["predicted_ids"])):
    ...
    json_str = json.dumps(
        ...
        ensure_ascii=False)
    #print(json_graph.tree_data(graph, (0, 0))['children'])
    #print(dfs(json_graph.tree_data(graph, (0, 0))))
    for ib in dfs(json_graph.tree_data(graph, (0, 0))):
        str = ''
        for jb in ib:
            if (jb == 'START'):
                continue
